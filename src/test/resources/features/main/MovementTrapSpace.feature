...
Feature: Move the player into a trap space
  ...
  Scenario: Move right into trap space
    ...
    And the player lost 1 health point
  ...
  Scenario: Move down, back up, then back down into trap space
    When the player moves down
    Then the player moves up
    Then the player moves down   
    Then the player is located at (4, 4)
    And the player lost 2 health points
