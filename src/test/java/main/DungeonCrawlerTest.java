...
public class DungeonCrawlerTest {
    ...
    public void setUp() {
        ...
        dungeonCrawler = new DungeonCrawler(threadWrapper, gameEngine,
                gameFrame, framesPerSecondHandler);
        Mockito.when(gameEngine.isExit()).thenReturn(false, true);
        Mockito.when(framesPerSecondHandler.hasEnoughTimeElapsed())
                .thenReturn(true);
        Mockito.when(
                framesPerSecondHandler.calculateSleepDurationInMilliSeconds())
                .thenReturn(SLEEP_TIME);
    }
    ...
    @Test
    public void set_exit_when_thread_interrupted_exception()
            throws InterruptedException {
        InterruptedException interruptedException = Mockito
                .mock(InterruptedException.class);
        Mockito.doThrow(interruptedException).when(threadWrapper)
                .sleep(SLEEP_TIME);
        dungeonCrawler.run();
    ...
    @Test
    public void run_and_exit_only_when_set_exit_true()
            throws InterruptedException {
