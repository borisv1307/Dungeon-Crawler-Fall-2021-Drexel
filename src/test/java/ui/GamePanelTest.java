...
public class GamePanelTest {
    ...
    public void setUp() throws Exception {
        ...
        Mockito.when(gameEngine.getLevelHorizontalDimension())
                .thenReturn(horizontalDimension);
        Mockito.when(gameEngine.getLevelVerticalDimension())
                .thenReturn(verticalDimension);
    ...
    public void paint() {
        ...
        Mockito.when(gameEngine.getPlayerXCoordinate())
                .thenReturn(playerXCoordinate);
        Mockito.when(gameEngine.getPlayerYCoordinate())
                .thenReturn(playerYCoordinate);
        gamePanel.paint(graphics);
        Mockito.verify(tilePainter).paintTiles(graphics, gameEngine, tileWidth,
                tileHeight);
        Mockito.verify(tilePainter).paintPlayer(graphics, playerXCoordinate,
                playerYCoordinate, tileWidth, tileHeight, TileType.PLAYER);
    }
    ...
    public void update() {
        ...
        Mockito.verify(graphics, Mockito.times(2)).drawImage(dbImage, 0, 0,
                gamePanel);
    }
