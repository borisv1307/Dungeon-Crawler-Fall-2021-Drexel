...
public class TilePainterTest {
    ...
    public void paint_tiles() {
        ...
        Mockito.when(game.getTileFromCoordinates(1, 1))
                .thenReturn(TileType.NOT_PASSABLE);
        Mockito.when(game.getTileFromCoordinates(
                AdditionalMatchers.not(Matchers.eq(1)),
                AdditionalMatchers.not(Matchers.eq(1))))
                .thenReturn(TileType.PASSABLE);
        ...
        inOrder.verify(graphics)
                .setColor(TileColorMap.get(TileType.NOT_PASSABLE));
