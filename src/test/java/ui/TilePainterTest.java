...
public class TilePainterTest {
	...
	public void paint_tiles() {
		...
		Mockito.when(game.getTileFromCoordinates(0, 2)).thenReturn(TileType.BOMB_POWER_UP);
		Mockito.when(game.getTileFromCoordinates(1, 0)).thenReturn(TileType.FIRE_POWER_UP);
		Mockito.when(game.getTileFromCoordinates(1, 1)).thenReturn(TileType.BREAKABLE);
		Mockito.when(game.getTileFromCoordinates(1, 2)).thenReturn(TileType.BOMB);
		...
		inOrder.verify(graphics).setColor(TileColorMap.get(TileType.BOMB_POWER_UP));
		inOrder.verify(graphics).fillRect(0, 40, 10, 20);
		inOrder.verify(graphics).setColor(TileColorMap.get(TileType.FIRE_POWER_UP));
		inOrder.verify(graphics).fillRect(10, 0, 10, 20);
		inOrder.verify(graphics).setColor(TileColorMap.get(TileType.BREAKABLE));
		inOrder.verify(graphics).fillRect(10, 20, 10, 20);
		inOrder.verify(graphics).setColor(TileColorMap.get(TileType.BOMB));
