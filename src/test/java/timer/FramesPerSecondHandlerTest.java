...
public class FramesPerSecondHandlerTest {
    ...
    public void setUp() {
        ...
        framesPerSecondHandler = new FramesPerSecondHandler(
                DESIRED_FRAMES_PER_SECOND, systemWrapper);
    }
    ...
    public void calculate_sleep_duration() {
        Mockito.when(systemWrapper.nanoTime()).thenReturn(
                1 * (long) FramesPerSecondHandler.MICROSECONDS_IN_A_SECOND);

        long actual = framesPerSecondHandler
                .calculateSleepDurationInMilliSeconds();

        long expected = FRAME_PERIOD_NS
                / FramesPerSecondHandler.MICROSECONDS_IN_A_SECOND;
    ...
    public void calculate_sleep_duration_zero() {
        Mockito.when(systemWrapper.nanoTime()).thenReturn((33
                - FramesPerSecondHandler.THREAD_SLEEP_LAG_DELAY)
